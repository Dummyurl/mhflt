...
//            mRedHintList = dao.getTaskHintByMonth(mSelYear, mSelMonth);
//        }
    }
    ...
    private void initAttrs(TypedArray array, int year, int month) {
        if (array != null) {
            ...
            mNormalDayColor = array.getColor(R.styleable.MonthCalendarView_month_normal_text_color, Color.parseColor("#666666"));
            mCurrentDayColor = array.getColor(R.styleable.MonthCalendarView_month_today_text_color, Color.parseColor("#F94529"));
...
//        drawHoliday(canvas);
    ...
    private int[] drawThisMonth(Canvas canvas) {
        ...
        for (int day = 0; day < monthDays; day++) {
            ...
            if (dayString.equals(String.valueOf(mSelDay)) && mSelDay > 0) {
    ...
    private void drawHoliday(Canvas canvas) {
        if (mIsShowHolidayHint) {
            ...
            for (int i = 0; i < mHolidays.length; i++) {
                int column = i % 7;
                int row = i / 7;
                rectF.set(mColumnSize * (column + 1) - mRestBitmap.getWidth() - distance - mColumnSize / 4 - mColumnSize / 9,
                        mRowSize * row + distance + mRowSize / 2 + mRowSize / 5,
                        mColumnSize * (column + 1) - distance - mColumnSize / 4 - mColumnSize / 9,
                        mRowSize * row + mRestBitmap.getHeight() + distance + mRowSize / 2 + mRowSize / 5);
                if (mHolidays[i] == 1) {
    ...
    private void drawHintCircle(Canvas canvas) {
        if (mIsShowHint && mRedHintList != null && mRedHintList.size() > 0) {
            List<Integer> hints = mRedHintList;
            ...
            for (int day = 0; day < monthDays; day++) {
                ...
                float circleY = (float) (mRowSize * row + mRowSize * 0.75 + 20);
                canvas.drawCircle(circleX, circleY, mCircleRadius, mPaint);
            }
        }

        if (mIsShowHint && mBlueHintList != null && mBlueHintList.size() > 0) {
            List<Integer> hints = mBlueHintList;
