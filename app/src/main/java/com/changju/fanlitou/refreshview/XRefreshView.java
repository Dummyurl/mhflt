...
import com.changju.fanlitou.refreshview.callback.IHeaderCallBack;
    ...
    private void stopLoadMore(final boolean hideFooter, final int scrollBackDuration) {
        if (needAddFooterView()) {
            if (mPullLoading) {
                ...
                if (mPinnedTime >= 100) {// 在加载更多完成以后，只有mPinnedTime大于100ms才生效，不然效果不好
                    postDelayed(new Runnable() {
