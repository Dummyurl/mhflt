...
public class AddBankCardDialog extends DialogFragment{
    ...
    public static AddBankCardDialog newInstance(AddBankInfo info) {

        AddBankCardDialog.info = info;
    ...
    private void initView(Dialog dialog) {
        ...
        BankCardAdapter adapter = new BankCardAdapter(info.getBanks());
        adapter.bindToRecyclerView(mRecyclerView);
        adapter.setOnItemClickListener(new BaseQuickAdapter.OnItemClickListener() {
            @Override
            public void onItemClick(BaseQuickAdapter adapter, View view, int position) {
                ((BankAddFragment)getParentFragment()).
                        setResultFinish(null,null,info.getBanks().get(position));
                dismiss();
            }
        });
    ...
    private class BankCardAdapter extends BaseQuickAdapter<AddBankInfo.BanksBean,BaseViewHolder>{

        public BankCardAdapter( @Nullable List<AddBankInfo.BanksBean> data) {
            super(R.layout.recycler_item_add_bank_card, data);
        }

        @Override
        protected void convert(BaseViewHolder helper, AddBankInfo.BanksBean item) {
            helper.setText(R.id.tv_bank_title,item.getName());
            ImageView logo = helper.getView(R.id.iv_logo);
            Glide.with(context).load(item.getLogo()).into(logo);
        }
    }
}
