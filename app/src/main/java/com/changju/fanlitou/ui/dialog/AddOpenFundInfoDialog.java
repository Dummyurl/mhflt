...
import com.changju.fanlitou.activity.fund.OpenFundActivity;
...
public class AddOpenFundInfoDialog extends DialogFragment implements View.OnClickListener {

    private NoScrollViewPager mViewPager;
    private TabLayout mTabLayout;

    private OpenFundActivity context;

    private List<OpenFundAccountBean.ProvinceListBean> province_list;

    public static AddOpenFundInfoDialog newInstance(
            List<OpenFundAccountBean.ProvinceListBean> province_list) {
        AddOpenFundInfoDialog addOpenFundInfoDialog = new AddOpenFundInfoDialog();
        addOpenFundInfoDialog.province_list = province_list;
        return addOpenFundInfoDialog;
    }
    ...
    public void onAttach(Activity activity) {
        ...
        context = (OpenFundActivity) activity;
    }
    ...
    List<OpenFundAccountBean.ProvinceListBean> mOneList;
    //一级tab选中
    int mCurrentOneSelect = -1;

    private CityAdapter mCityAdapter;
    List<OpenFundAccountBean.ProvinceListBean.CitysBean> mTwoList;
    ...
    private void initView(Dialog dialog) {
        ...
        mProvinceAdapter.setOnItemClickListener(new BaseQuickAdapter.OnItemClickListener() {
            ...
            public void onItemClick(BaseQuickAdapter adapter, View view, int position) {
                OpenFundAccountBean.ProvinceListBean one = mOneList.get(position);

                mCurrentOneSelect = position;

                mTwoList = one.getCitys();
    ...
    private class ProvinceAdapter extends BaseQuickAdapter<OpenFundAccountBean.ProvinceListBean, BaseViewHolder> {

        public ProvinceAdapter(List<OpenFundAccountBean.ProvinceListBean> data) {
            super(R.layout.recycler_item_add_bank_info, data);
        }

        @Override
        protected void convert(BaseViewHolder helper,
                               OpenFundAccountBean.ProvinceListBean item) {
            helper.setText(R.id.tv_content,item.getName());
        }
    }

    private class CityAdapter extends BaseQuickAdapter<OpenFundAccountBean.ProvinceListBean.CitysBean,BaseViewHolder>{

        public CityAdapter( @Nullable List<OpenFundAccountBean.ProvinceListBean.CitysBean> data) {
            super(R.layout.recycler_item_add_bank_info, data);
        }

        @Override
        protected void convert(BaseViewHolder helper, OpenFundAccountBean.ProvinceListBean.CitysBean item) {
            helper.setText(R.id.tv_content,item.getName());
        }
    }
